...
export default function DepartmentPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        const departmentToUpdate = departments.find(dept => dept.id === editingId);
        if (!departmentToUpdate) {
          throw new Error("Department not found");
        }
        
        await api.put(`/departments/by-email/${adminEmail}/${departmentToUpdate.employee.id}`, formData);
      } else {
...
//     try {
//       if (editingId) {
//         // Find the department being edited to get the employee ID
//         const departmentToUpdate = departments.find(dept => dept.id === editingId);
//         if (!departmentToUpdate) {
//           throw new Error("Department not found");
//         }
...
//       setAdminEmail(response.data.email);
//       setFormData(prev => ({ ...prev, adminEmail: response.data.email }));
//       setDeleteEmail(response.data.email);
//     } catch (error: any) {
//       console.error("Error fetching admin email:", error);
//       setError("Failed to fetch admin information. Please check your authentication.");
//     } finally {
...
//     if (!confirm(`Are you sure you want to delete all departments for admin: ${deleteEmail}?`)) return;
    
//     try {
//       // This would require a backend endpoint for bulk deletion by email
//       // For now, we'll delete each department one by one
//       const departmentsToDelete = departments.filter(dept => dept.admin.Email === deleteEmail);
      
//       for (const dept of departmentsToDelete) {
//         await api.delete(`/departments/by-email/${deleteEmail}`);
//       }
      
//       setDeleteEmail("");
//       fetchDepartments();
//       alert(`Deleted ${departmentsToDelete.length} departments for admin ${deleteEmail}`);
//     } catch (error) {
//       console.error("Failed to delete departments by email:", error);
//       setError("Failed to delete departments by email. Please try again.");
//     }
//   };
...
//           <h2 className="text-xl font-semibold mb-2">Delete Departments by Admin Email</h2>
...
//             This will delete all departments for the specified admin.
//           </p>
//         </div>
...
//                     </span>
//                   </td>
//                   <td className="px-6 py-4 whitespace-nowrap">{dept.admin.Email}</td>
//                   <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
//                     <button
//                       onClick={() => handleEdit(dept)}
//                       className="text-indigo-600 hover:text-indigo-900 mr-3"
